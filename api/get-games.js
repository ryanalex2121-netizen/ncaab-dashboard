...
const season = `${year}`; // e.g., 2025 --- We are testing if the API uses a single year
...
const aiProjections = [
    ...
    // (Keeping a few for testing, but they won't match live games)
    {
        id: 1, team1: "Louisville", team2: "SC State",
        projectedScore: { team1: 85, team2: 63 }, 
        analysis: "Mock data... will not match."
    },
    {
...
export default async function handler(request, response) {
    ...
    if (!API_SPORTS_KEY) {
        return response.status(500).json({ 
            ...
            details: "API_SPORTS_KEY was not found in Vercel Environment Variables." 
        });
    }
    ...
    try {
        ...
        // --- !! NEW ERROR HANDLING !! ---
        // Check for 404s, 403s, etc.
        if (!spreadResponse.ok) {
            throw new Error(`API-Sports (Spreads) Error: ${spreadResponse.status} ${spreadResponse.statusText}. Check URL: ${SPREAD_URL}`);
        }
        if (!totalResponse.ok) {
            throw new Error(`API-Sports (Totals) Error: ${totalResponse.status} ${totalResponse.statusText}. Check URL: ${TOTAL_URL}`);
        }

        const spreadData = await spreadResponse.json();
        const totalData = await totalResponse.json();

        // Check for errors inside the JSON response
        if (spreadData.errors?.token || totalData.errors?.token) {
             throw new Error("API Key is invalid or not authorized. Check your Vercel Environment Variables.");
        }
        if (spreadData.errors && Object.keys(spreadData.errors).length > 0) {
            throw new Error(`API-Sports returned an error: ${JSON.stringify(spreadData.errors)}`);
        }

        // --- !! NEW CHECK for 0 results !! ---
        // This is a common 404 "soft error" - the request is valid, but there's no data.
        if (spreadData.results === 0 || totalData.results === 0) {
            throw new Error(`No odds found on API-Sports for league 886, season ${season}, date ${apiDate}. (This is normal if no games are scheduled or odds aren't posted yet).`);
        }
    ...
    aiData.forEach(aiGame => {
        ...
        if (apiSpreadGame && apiTotalGame) {
            ...
            if (marketSpreadValue && marketTotalValue) {
                ...
                // The total value might be "Over 157.5" or just "157.5", so this regex handles both.
